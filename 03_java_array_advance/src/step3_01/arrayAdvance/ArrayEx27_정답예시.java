	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			for (int i = 0; i < game.length; i++) {
				...
				else 				System.out.print(game[i]+" "); //캐릭터 아니면 빈칸
			}
			System.out.println();
			...
			if (move == 1) {				//만약 1(왼쪽)이면
				...
				if (playerIdx -1 >= 0 ) {			//만약 playerIdx(???????) - 1이 0보다 크거나 똑같으면
					...
					game[playerIdx] = 0;					 //playerIdx번째 요소 (원래 캐릭터가 있었던 곳)을 2로 바꿔준다
					playerIdx--;							 //playerIdx(?????????)를 빼줌... 왜?????
														     //playerIdx가 혹시 이동 수? 아님 현재 위치 수?
				...
				if (playerIdx + 1 < 9 ) {					 //만약 playerIdx + 1이 0보다 작으면 (9 이상 못 지나가게 하려고)
